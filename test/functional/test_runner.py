...
def main():
    ...
    tmpdir = "%s/bongwater_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and bongwaterd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "bongwaterd"]) is not None:
            print("%sWARNING!%s There is already a bongwaterd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "BONGWATERD" not in os.environ:
        os.environ["BONGWATERD"] = build_dir + '/src/bongwaterd' + exeext
        os.environ["BONGWATERCLI"] = build_dir + '/src/bongwater-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `bongwater-cli help` (`rpc_interface.txt`).
